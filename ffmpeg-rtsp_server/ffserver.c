...
extern int pickup_pkt(AVPacket *pkt);
typedef struct {
...
static AVFormatContext *_s_fmt_in = NULL;
static char *_s_config = NULL;
static int _s_stream_index;

static HTTPContext *first_http_ctx = NULL;
...
static int _s_should_exit;
static int _s_rtsp_server_fd;
...
static int http_server(void)
{
    ...
    if (config.rtsp_addr.sin_port) {
        ...
        _s_rtsp_server_fd = rtsp_server_fd;
        if (rtsp_server_fd < 0) {
            av_free(poll_table);
            closesocket(server_fd);
            return -1;
...
static int http_prepare_data(HTTPContext *c)
{
    ...
    case HTTPSTATE_SEND_DATA:
        ...
        redo:
            ...
            ret = pickup_pkt(&pkt);
            if ((ret != 0) || (pkt.stream_index != _s_stream_index)) {
                msleep(30);
                goto redo;
            }
            if (ret < 0) {
...
static void build_file_streams(void)
{
    ...
    for(stream = config.first_stream; stream; stream = stream_next) {
        ...
        if (stream->stream_type == STREAM_TYPE_LIVE &&
            ...
            extract_mpeg4_header(_s_fmt_in);
            
            for(i=0;i<_s_fmt_in->nb_streams;i++)
                add_av_stream1(stream, _s_fmt_in->streams[i]->codec, 1);
...
int _rtsp_main(int argc, char **argv)
{
    ...
    if ((ret = ffserver_parse_ffconfig(config.filename, &config, _s_config)) < 0) {
    ...
    close(_s_rtsp_server_fd);
    HTTPContext *c = first_http_ctx;
    HTTPContext *c_next;
    while (c) {
        c_next = c->next;
        close_connection(c);
...
void *rtsp_main(void *arg)
{
    ...
    Trans_data *trans_data = (Trans_data*)arg;
    _s_config = trans_data->config;
    _s_stream_index = trans_data->stream_index;
    _s_fmt_in = trans_data->fmt_ctx;
    char *argv[2];
    argv[0] = "decoder";
    argv[1] = trans_data->config_filename;
    _rtsp_main(2, argv);
    return NULL;
}
